...
router.get('/', async function (req, res, next) {
  ...
  const docs = await SystemState.find({
    AccountID: mongoose.Types.ObjectId(account_doc._id)
  });
  if (!docs) return res.status(404).send('Account Not Found!');else return res.send(docs);
});
router.post('/', async function (req, res, next) {
  ...
  const previous = await SystemState.findOne({
    AccountID: mongoose.Types.ObjectId(accountID)
  });
  const monitor = await Monitors.findOne({
    APIKey: apiKey
  });
...
function parseData(data, accountID, previous, monitor) {
  return {
    AccountID: mongoose.Types.ObjectId(accountID),
    MonitorID: monitor._id,
    CPU: previous ? [...previous.CPU, { ...data.cpu,
      time: new Date()
    }] : [{ ...data.cpu,
      time: new Date()
    }],
