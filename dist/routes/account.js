...
export async function login(req, res, next) {
  ...
  const accoun_doc = await Account.findOne({
    UserName: username
  });
  return res.send({
...
export async function register(cryptr, req, res, next) {
  ...
  if (!username || !password) return res.send(400, 'Bad Request');
  const doc = await Account.findOne({
    UserName: username
  });
  if (doc) return res.send(400, 'User Exists');
  ...
  await account.save();
  const newDoc = await Account.findOne({
    UserName: username
  });
  return res.send({
